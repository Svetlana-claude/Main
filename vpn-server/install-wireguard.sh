...
if command -v ufw >/dev/null 2>&1 && [[ "$ufw_state" == *active* ]]; then
...
# ⚠️ Вывод `ss` читается в переменную, а не подаётся в `grep -q` конвейером:
# `grep -q` выходит по первому совпадению и закрывает канал, `ss` получает
# SIGPIPE и завершается с кодом 141, а `pipefail` делает этот код кодом всего
# конвейера. Установщик объявил бы, что порт никто не слушает, при живом
# сервере. На этом же образце 11–12.09.2026 двое суток терялись выгрузки базы:
# там проверка не находила таблицу и удаляла годный файл.
listening=$(ss -lnup)
case "$listening" in
    *":$WG_PORT "*) ;;
    *) die "порт $WG_PORT/udp никто не слушает" ;;
esac
