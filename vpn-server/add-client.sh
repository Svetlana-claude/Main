...
# неотличимо от флага.
printf '%s' "$NAME" | grep -qE '^[A-Za-z0-9][A-Za-z0-9_-]{0,31}$' \
    || die "имя может состоять из латиницы, цифр, дефиса и подчёркивания, первым знаком буква или цифра (до 32 знаков)"
