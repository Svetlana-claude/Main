...
#
# Смена почты учётной записи (писем об истечении), отдельным режимом:
#     sudo bash infra/tls-letsencrypt.sh --email you@example.com
#
...
# ── Смена почты учётной записи ────────────────────────────────────────
# Отдельный режим: sudo webui-tls-issue --email адрес@почта
#
# Почта хранится не в сертификате, а в учётной записи ACME, и нужна ради писем
# об истечении: они приходят, если автопродление молча сломалось. При выпуске
# довод -m учитывается только в момент регистрации учётной записи; если она уже
# создана, поменять адрес можно лишь этой командой.
#
# Почему режим здесь, а не строкой «certbot update_account *» в списке
# разрешённых команд: у certbot есть --pre-hook и --deploy-hook, выполняющие
# произвольную команду от root, и --config-dir, которым можно писать от root
# куда угодно. Звёздочка в списке разрешила бы и это. Здесь же довод один,
# и он проверяется на вид почтового адреса — флаг в него не спрятать.
if [ "${1:-}" = "--email" ]; then
    ACCOUNT_EMAIL="${2:-}"
    if [ "$(id -u)" -ne 0 ]; then
        echo "нужен root: sudo webui-tls-issue --email адрес@почта" >&2
        exit 1
    fi
    if [[ ! "$ACCOUNT_EMAIL" =~ ^[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}$ ]]; then
        echo "ОТКАЗ: «$ACCOUNT_EMAIL» не похоже на почтовый адрес." >&2
        exit 1
    fi
    if [ $# -gt 2 ]; then
        echo "ОТКАЗ: лишние доводы после адреса — ожидается ровно один." >&2
        exit 1
    fi
    echo "=== Смена почты учётной записи на $ACCOUNT_EMAIL ==="
    # --no-eff-email обязателен: без него certbot предлагает передать адрес
    # в рассылку EFF. Подписывать чужую почту на рассылку молча нельзя.
    certbot update_account -m "$ACCOUNT_EMAIL" --non-interactive --no-eff-email
    echo
    echo "=== Что записано в учётной записи ==="
...
if [ ${#DOMAINS[@]} -eq 0 ]; then
    ...
    echo "       смена почты: sudo bash infra/tls-letsencrypt.sh --email адрес@почта" >&2
    exit 1
fi
if [ "$(id -u)" -ne 0 ]; then
